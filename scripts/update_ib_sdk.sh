...
[ -f $GITHUB_ENV ] && echo "GITHUB_ENV $GITHUB_ENV exists" || {
  ls -l `dirname $GITHUB_ENV`
  echo "GITHUB_ENV $GITHUB_ENV not exists"
  exit 1
}

echo "update_ib_sdk: exporting env vars ... KOUHJ_SRC_DIR=${KOUHJ_SRC_DIR}"
