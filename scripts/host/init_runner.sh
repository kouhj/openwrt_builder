...
setup_envs() {
  ...
  BUILDER_MOUNT_OPTS="
    ...
    -v '${HOST_WORK_DIR}/dl_cache:${BUILDER_WORK_DIR}/dl_cache'
  ...
  persistent_env_set OPENWRT_COMPILE_DIR OPENWRT_SOURCE_DIR OPENWRT_CUR_DIR DL_CACHE_DIR
...
prepare_target() {
  ...
  # Load settings
  NECESSARY_SETTING_VARS=( OPT_DEBUG BUILDER_NAME BUILDER_TAG REPO_URL REPO_BRANCH REPO_VERSION \
                           CONFIG_TARGET_BOARD CONFIG_TARGET_SUBTARGET CONFIG_TARGET_ARCH_PACKAGES \
                           PRE_BUILT_PACKAGES_SITE_URL  )
  OPT_UPLOAD_CONFIG='1'
